...
open(HAP, "/group/im-lab/nas40t2/egamazon/annotation/goodSnpsIntersection.WTCCC.txt"); ##QC'd genotyped SNPs in WTCCC, used for imputation by Eric
...
for(my $i = 1; $i <= 22; $i++){
    ...
    my $bimhandle = "BIM" . $i;
    open($snpxidhandle, ">DGN-imputed-for-PrediXcan/DGN.imputed_maf0.05_R20.8.wtcccGenotypedSNPs.chr${i}.SNPxID");
    open($bimhandle, ">DGN-imputed-for-PrediXcan/DGN.imputed_maf0.05_R20.8.wtcccGenotypedSNPs.chr${i}.bim");
    open($mlinfohandle, ">DGN-imputed-for-PrediXcan/DGN.imputed_maf0.05_R20.8.wtcccGenotypedSNPs.chr${i}.mlinfo");
    open(ID, ">DGN-imputed-for-PrediXcan/DGN.imputed_maf0.05_R20.8.wtcccGenotypedSNPs.ID.list");
    open(SNP, ">DGN-imputed-for-PrediXcan/DGN.imputed_maf0.05_R20.8.wtcccGenotypedSNPs.chr${i}.SNP.list");
	...
	my ($chr, $pos, $id, $ref, $alt, $qual, $filter, $info, $format, @genos) = split(/\s+/);
...
for(my $i = 1; $i <= 22; $i++){
    ...
    print R "dat<-scan(\"DGN-imputed-for-PrediXcan/DGN.imputed_maf0.05_R20.8.wtcccGenotypedSNPs.chr${i}.SNPxID\")\n";
    print R "gtidlist<-scan(\"DGN-imputed-for-PrediXcan/DGN.imputed_maf0.05_R20.8.wtcccGenotypedSNPs.ID.list\",\"character\")\n";
    ...
    system("paste -d\' \' intro t.dos.chr${i} > DGN-imputed-for-PrediXcan/DGN.imputed_maf0.05_R20.8.wtcccGenotypedSNPs.chr${i}.mldose");
}
